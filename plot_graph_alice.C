...
const char *channeltext2 = "W #rightarrow e + #nu";
const char *channeltext2_p = "W^{+} #rightarrow #mu^{+} + #nu";
const char *channeltext2_m = "W^{-} #rightarrow #mu^{-} + #nu";
const char *channeltext3 = "ALICE data";
const char *channeltext3_p = "ALICE data";
const char *channeltext3_m = "ALICE data";
...
TGraphAsymmErrors* theory_errors(const char* gbasename, int channel_number, int nbins, const char* pdfname, bool doct10err=true)
{
   ...
   // if (doAlice) filename = string(label) + string("_wide.dta");
...
TGraphErrors* theory_alice_plus(bool npdf=false)
{
   ...
   // values from Hannu
   if (npdf) {y[0] = 47.12; y[1] = 7.01;}
   else {y[0] = 54.38; y[1] = 7.54;}
   for (int i=0; i<2; i++) y[i]=y[i]*LUMI;//*(2*ex[i]);
   
   double ey[2] = {0,0};

   // values from Hannu
   if (npdf) {ey[0] = 4.06; ey[1] = 0.42;}
   else {ey[0] = 2.40; ey[1] = 0.40;}
   for (int i=0; i<2; i++) ey[i]=ey[i]*LUMI;//*(2*ex[i]);

   TGraphErrors *gr = new TGraphErrors(2,x,y,ex,ey);
   return gr;
}

TGraphErrors* theory_alice_minus(bool npdf=false)
{
   double x[2] = {-3.245,3.245};
   double ex[2] = {0.75,0.75};
   ...
   // values from Hannu
   if (npdf) {y[0] = 34.38; y[1] = 27.21;}
   else {y[0] = 39.45; y[1] = 28.34;}
   for (int i=0; i<2; i++) y[i]=y[i]*LUMI;//*(2*ex[i]);

   double ey[2] = {0,0};

   // values from Hannu
   if (npdf) {ey[0] = 2.95; ey[1] = 1.42;}
   else {ey[0] = 2.00; ey[1] = 1.38;}
   for (int i=0; i<2; i++) ey[i]=ey[i]*LUMI;//*(2*ex[i]);
...
TGraphErrors* theory_alice_CA(bool npdf=false)
{
   double x[2] = {-3.245,3.245};
   double ex[2] = {0.75,0.75};
   double y[2];

   if (npdf) {y[0] = 0.155; y[1] = -0.590;}
   else {y[0] = 0.159; y[1] = -0.580;}

   double ey[2];

   if (npdf) {ey[0] = 2.29e-2; ey[1] = 1.6e-2;}
   else {ey[0] = 2.28e-2; ey[1] = 1.47e-2;}

   // cout << "th " << npdf << ": 0 " << y[0] << ", 1 " << y[1] << endl;
...
TGraphErrors* data_alice_CA_stat()
{
   double x[2] = {-3.245,3.245};
   double ex[2] = {0.75,0.75};
   double yp[2] = {86.2,12.8};
   double eyp[2] = {5.4,2.0};
   double ym[2] = {82.9,75.9};
   double eym[2] = {5.6,5.4};
   for (int i=0; i<2; i++) 
   {
      yp[i]=yp[i]*LUMI/acc_alice_plus[i]/(2*ex[i]);
      eyp[i]=sqrt(pow(eyp[i]*(ex[i])*LUMI/acc_alice_plus[i],2)+pow(yp[i]*acc_alice_plus_err[i]/acc_alice_plus[i],2))/(2*ex[i]);
      ym[i]=ym[i]*LUMI/acc_alice_minus[i]/(2*ex[i]);
      eym[i]=sqrt(pow(eym[i]*(ex[i])*LUMI/acc_alice_minus[i],2)+pow(ym[i]*acc_alice_minus_err[i]/acc_alice_minus[i],2))/(2*ex[i]);
   }

   double y[2], ey[2];
   for (int i=0; i<2; i++)
   {
      y[i] = (yp[i]-ym[i])/(yp[i]+ym[i]);
      double Aa = yp[i];
      double Ba = ym[i];
      double dAa = eyp[i];
      double dBa = eym[i];
      ey[i] = sqrt((4*Ba*Ba/pow(Aa+Ba,4))*dAa*dAa + (4*Aa*Aa/pow(Aa+Ba,4)*dBa*dBa));
      cout << "alice ch " << i << ": " << y[i] << " " << ey[i] << endl;
   }
...
TGraphErrors* data_alice_CA_syst()
{
   double x[2] = {-3.245,3.245};
   double ex[2] = {0.75,0.75};
   double yp[2] = {86.2,12.8};
   double eyp[2] = {8.6,2.8};
   double ym[2] = {82.9,75.9};
   double eym[2] = {14.8,10.4};
   for (int i=0; i<2; i++) 
   {
      yp[i]=yp[i]*LUMI/acc_alice_plus[i]/(2*ex[i]);
      eyp[i]=sqrt(pow(eyp[i]*(ex[i])*LUMI/acc_alice_plus[i],2)+pow(yp[i]*acc_alice_plus_err[i]/acc_alice_plus[i],2))/(2*ex[i]);
...
void plot_graph_1file(const char* fname="graph.root", const char *gbasename="gyields", int channel_number=0, bool reverteta=false, double scale=1.)
{
   ...
   if (string(gbasename)=="gch") galice_stat = data_alice_CA_stat();
   TGraphErrors *galice_syst = (string(gbasename)=="gyieldsp") ? data_alice_plus_syst() : data_alice_minus_syst();
   if (string(gbasename)=="gch") galice_syst = data_alice_CA_syst();
   ...
   TGraphErrors *galice_th = (string(gbasename)=="gyieldsp") ? theory_alice_plus(false) : theory_alice_minus(false);
   if (string(gbasename)=="gch") galice_th = theory_alice_CA(false);
   TGraphErrors *galice_th_eps09 = (string(gbasename)=="gyieldsp") ? theory_alice_plus(true) : theory_alice_minus(true);
   if (string(gbasename)=="gch") galice_th_eps09 = theory_alice_CA(true);
   ...
   if (doAlice)
   {
      ...
      galice_th->SetFillColor(gColorCT10_fill);
      // galice_th->SetFillStyle(3002);
      galice_th->SetFillStyle(1001);
      // galice_th->Draw("AL3");
      galice_th->Draw("2");
      galice_th->GetXaxis()->SetLimits(xmin,xmax);
      galice_th->GetYaxis()->SetRangeUser(ymin,ymax);
      galice_th->GetXaxis()->SetTitle(xlabel.c_str());
      if (twopads) galice_th->GetXaxis()->SetTitle("");
      galice_th->GetXaxis()->SetNdivisions(505);
      galice_th->GetXaxis()->SetTitleSize(lTextSize);
      galice_th->GetXaxis()->SetLabelSize(lTextSize);
      galice_th->GetYaxis()->SetTitle(ylabel.c_str());
      galice_th->GetYaxis()->SetTitleSize(lTextSize);
      galice_th->GetYaxis()->SetLabelSize(lTextSize);
      // galice_th->Draw("AL3");
      galice_th->Draw("2");
      TGraphErrors *galice_th2 = new TGraphErrors(*galice_th);
      setErrorY(galice_th2,0);
      galice_th2->Draw("Z");

      galice_th_eps09->SetLineColor(gColorEPS09);
      galice_th_eps09->SetLineStyle(9);
      galice_th_eps09->SetLineWidth(2);
      galice_th_eps09->SetFillColor(gColorEPS09);
      galice_th_eps09->SetFillStyle(3345);
      // galice_th_eps09->Draw("L3");
      galice_th_eps09->Draw("2");
      cout << galice_th_eps09->GetY()[0] << " " << galice_th_eps09->GetY()[1] << endl;
      TGraphErrors *galice_th_eps092 = new TGraphErrors(*galice_th_eps09);
      setErrorY(galice_th_eps092,0);
      galice_th_eps092->Draw("Z");
   }
   ...
   if (doAlice)
   {
      galice_syst->SetLineColor(kBlue+1);
      galice_syst->SetMarkerColor(kBlue+1);
      galice_syst->SetMarkerStyle(kFullStar);
      galice_syst->SetFillStyle(0);
      galice_syst->SetLineWidth(2);
      galice_syst->Draw("||");

      galice_stat->SetLineColor(kBlue+1);
      galice_stat->SetMarkerColor(kBlue+1);
